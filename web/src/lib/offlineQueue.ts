...
// Browser-only: ported from the Expo app's NetInfo listener + AsyncStorage
// singleton, which ran this same module-top-level wiring unconditionally.
// Here that would also execute during Next.js's server-side render of the
// 'use client' screens that import this module — `window`/`localStorage`
// don't exist there, so this is guarded to only wire up in the browser.
